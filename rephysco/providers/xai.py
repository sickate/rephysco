"""XAI provider implementation for the Rephysco LLM System.

This module implements the XAI provider, which supports models like Grok-2, Grok-2-Vision, etc.
"""
...
from ..config import XAI as XAIConfig
...
class XAIProvider(BaseProvider):
    """Provider implementation for XAI API (Grok models)."""
    
    # Models with vision capabilities
    VISION_MODELS = [XAIConfig.GROK_2_VISION]
    ...
    def __init__(self, api_key: Optional[str] = None, **kwargs):
        ...
        """
        api_key = api_key or XAIConfig.API_KEY
        super().__init__(api_key=api_key, **kwargs)
        
        # Initialize the OpenAI client with XAI base URL
        self.client = AsyncOpenAI(
            api_key=self.api_key,
            base_url=kwargs.get("base_url", XAIConfig.BASE_URL)
        )
        
        # Store the current model (used for multimodal formatting)
        self._current_model = XAIConfig.GROK_2
        
        if not self.validate_api_key():
            raise AuthenticationError("XAI API key is required")
    
    @property
    def default_model(self) -> str:
        """Get the default model for XAI."""
        return XAIConfig.GROK_2
        ...
        for message in messages:
            ...
            if message.images and message.content is not None:
                ...
                if self._current_model in self.VISION_MODELS:
                    # Format for vision models according to XAI documentation
                    content_parts = []
        ...
        **kwargs
        ...
        try:
            ...
            if "authentication" in error_message.lower() or "api key" in error_message.lower():
                ...
                raise ProviderError(f"XAI API error: {error_message}")
    
    async def _handle_streaming_response(self, response) -> AsyncGenerator[StreamChunk, None]:
        """Handle a streaming response from the XAI API.
        
        Args:
            response: The streaming response from XAI
    ...
    def parse_response(self, response: ChatCompletion) -> ModelResponse:
        """Parse a response from the XAI API.
        
        Args:
            response: The response from XAI
        ...
        try:
            ...
            raise ProviderError(f"Failed to parse XAI response: {e}")
    
    def parse_stream_chunk(self, chunk: ChatCompletionChunk) -> StreamChunk:
        """Parse a streaming chunk from the XAI API.
        
        Args:
            chunk: The chunk from XAI
